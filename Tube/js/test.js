...
class App {
    ...
    createMain(videos, comments) {
        const main = new Aside(videos, comments);
        return main.element;
    }
    ...
    setupRelatedVideos(videos) {
        const switcherItems = Array.from(this.container.querySelectorAll('.mini-video-item'));

        switcherItems.forEach((switcherItem, index) => {
...
class RelatedVideosSection {
    ...
    createRelatedVideosSection(videos) {
        ...
        switcherList.className = 'mini-videos';

        const switcherItems = videos.map((video) => {
            const switcherItem = new Switcher(video);
            return switcherItem.element;
        });

        switcherList.append(...switcherItems);
        relatedVideos.append(relatedVideosTitle, switcherList);
...
class Switcher {
    constructor(video) {
        this.element = this.createSwitcher(video);
    }

    createSwitcher(video) {
        const switcherItem = document.createElement('li');
        switcherItem.className = 'mini-video-item';

        const switcherLink = document.createElement('a');
        switcherLink.href = '#';

        const switcherWrapper = document.createElement('div');
        switcherWrapper.className = 'video-wrapper';

        const switcherThumb = document.createElement('img');
        switcherThumb.className = 'video-thumb';
        switcherThumb.src = video.thumbnail;

        switcherWrapper.appendChild(switcherThumb);
        switcherLink.appendChild(switcherWrapper);
        switcherItem.appendChild(switcherLink);
